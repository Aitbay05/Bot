...
def _configure_tesseract() -> None:
    ...
    Ескерту (нақты тестілеуде табылған проблема): .env файлы бір
    серверден екіншісіне (мыс. Windows dev-компьютерден Linux
    production серверіне) көшірілгенде, TESSERACT_CMD ескі/бөтен
    жол болып қалуы мүмкін (мыс. Windows жолы Linux серверде тұрып
    қалуы). Бұрын мұндай жағдайда pytesseract сол жарамсыз жолды
    сол күйінде қолданып, БҮКІЛ OCR (демек, бүкіл бот) үзіліп
    қалатын — әрі себебі логта бірден түсінікті болмайтын. Енді
    іске қосу алдында жолдың шынымен бар-жоғы тексеріледі; жоқ
    болса, ескерту жазылып, pytesseract-тың өзінің PATH арқылы
    автотабуына рұқсат етіледі (жүйелік tesseract бар болса, бот
    ...
    if TESSERACT_CMD:
        ...
        if os.path.isfile(TESSERACT_CMD):
            ...
            logger.warning(
                "TESSERACT_CMD=%s көрсетілген, бірақ бұл жол серверде табылмады "
                "(мүмкін .env басқа операциялық жүйеден көшірілген). "
                "Жүйелік tesseract (PATH) автоматты қолданылады.",
                TESSERACT_CMD,
            )
