...
## 🔴 ЖАҢА Round 2 — 2026-09-16 (шұғыл назар аудару керек)

### 0. КРИТИКАЛЫҚ: BOT_TOKEN Git тарихында жария болған, GitHub-қа жіберілген
- **Тексеру:** `git log --all -p -- env.example` арқылы расталды —
  нақты токен (`8840442455:AAHT...`) `env.example`-ге commit жасалған,
  ал репо remote-і `https://github.com/Aitbay05/Bot.git` көрсетілген.
- **Не істеу керек ҚАЗІР (кодты түзету жеткіліксіз):**
  1. Telegram-да **@BotFather** → `/revoke` (немесе `/token` → “Revoke
     current token”) арқылы осы токенді дереу жарамсыз етіңіз.
  2. Жаңа токен алып, тек жергілікті `.env`-ге қойыңыз (Git-ке
     ешқашан commit жасамаңыз).
  3. GitHub репозиторийін тексеріңіз: егер публикалық болса — дереу
     private етіңіз және `git filter-repo`/BFG арқылы тарихтан
     токенді толық өшіріңіз (жаңа commit жеткіліксіз).
  4. `credentials.json` (Google Service Account кілті) Git тарихында
     табылмады, бірақ сақтық үшін Google Cloud Console → IAM →
     Service Accounts бөлімінен соңғы қолданылу уақытын тексеріп,
     күмән болса кілтті rotate жасаңыз.
  5. `.env` ішіндегі `ADMIN_PASSWORD=Arbuz` — әлсіз, сөздікте бар
     сөз. Күшті, кездейсоқ парольге ауыстырыңыз.

### 1. Google Таблица тек 2 бағанға дейін қысқартылды (ФИО + Количество)
- **Файл:** `google_sheet.py`
- **Өзгеріс:** `HEADERS` енді `["ФИО", "Количество помощи"]`.
  "Заказы", "Последний заказ", "Последнее обновление" бағандары
  енді жазылмайды/жаңартылмайды (заказ нөмірлері мен уақыт
  `orders.db`/логта сақталады, қажет болса сол жерден алуға болады).
  Ескі кестелерде осы бағандардың тақырыптары автоматты түрде
  тазаланады (деректің өзі — ескі заказ нөмірлері — өшірілмейді,
  тек тақырып жолы тазаланады).

### 2. Formula injection (Google Sheets/Excel) қорғанысы
- **Файл:** `utils.py`, `clean_employee_name()`
- **Проблема:** Егер қызметкер caption-ге `=IMPORTXML(...)`,
  `=HYPERLINK(...)` сияқты `=`/`+`/`-`/`@`-дан басталатын мәтін
  жазса, бұл мән Google Таблицаға сол күйінде жазылып, диспетчер
  таблицаны ашқанда формула ретінде орындалуы мүмкін еді (сыртқы
  сұраныс жіберу, деректерді сыртқа шығару және т.б.).
- **Шешім:** мұндай таңбадан басталатын атаудың алдына апостроф
  (`'`) қосылады — Sheets/Excel мұны формула емес, қарапайым мәтін
  ретінде көрсетеді.

### 3. `/admin` логин/пароль салыстыруы timing-attack-қа осал болған
- **Файл:** `bot.py`
- **Проблема:** Қарапайым `==` салыстыруы таңба-таңбамен тексеріледі
  және дұрыс таңба саны артқан сайын аздап баяулайды — теориялық
  түрде парольді таңбалап тауып алуға мүмкіндік береді (network
  timing side-channel).
- **Шешім:** `hmac.compare_digest()` қолданылды — тұрақты уақытты
  салыстыру.
