...
# Google Таблицада/Excel-де "=", "+", "-", "@" таңбасынан басталатын
# ұяшық формула ретінде орындалуы мүмкін ("formula injection"). Егер
# қызметкер caption-ге әдейі "=IMPORTXML(...)" сияқты нәрсе жазса,
# диспетчер таблицаны ашқанда бұл формула автоматты орындалуы мүмкін.
...
def clean_employee_name(text: str) -> str:
    ...
    Также защищает от formula injection в Google Таблице/Excel: если
    результат начинается с символа, способного запустить формулу
    (=, +, -, @), перед ним добавляется апостроф — Google Sheets/Excel
    в этом случае показывают текст как есть, не выполняя его как формулу.
    """
    cleaned = " ".join(text.strip().split())
    if cleaned.startswith(_FORMULA_TRIGGER_CHARS):
        cleaned = "'" + cleaned
    return cleaned
